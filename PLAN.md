...
| A4  | Publisher id placeholder `randynorthrup` (derived from the owner's email).               | `vsce` needs a publisher id to package; the real marketplace publisher is not known yet.                                                                                                                                                                        | Trivial (one field).                            |
...
| A10 | Windows is the primary dev machine; Linux/macOS supported via CI.                        | Owner is on Windows 11. `muse` CLI is documented for macOS + Windows.                                                                                                                                                                                           | None.                                           |
...
| `@muse-code/sdk`                                                        | 1.3.0                             | Official MSP client. Developer Preview: "minor releases may alter APIs before 1.0" → exact pin, adapter isolated in one module, schema fingerprint checked at handshake.                                                                                                    |
...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| Q1  | Install the Muse Code CLI on this machine (`irm https://dev.meta.ai/install.ps1 \| iex`) so the MSP backend can be tested live? Do you have a Muse Code subscription and/or a Model API key? | Not installed. M2 ships with fake-transport tests only; live certification waits. |
| Q2  | Real marketplace publisher id and extension display name.                                                                                                                                    | `randynorthrup` / "Muse Spark Code (Unofficial)".                                 |
| Q3  | Should the Model API backend (M7) be in v1.0, or is CLI-only acceptable for first release?                                                                                                   | M7 stays in scope, after M6.                                                      |
...
| Q7  | Press F5, open the Muse Spark view, confirm the empty-state shell renders in your theme, and drop a screenshot into docs/certification/.                                                     | Integration test proves the panel opens; DOM verified under jsdom only.           |
...
- **Scope**: `muse` binary discovery (setting → PATH → documented install
  dirs); install guidance when absent (opens dev.meta.ai/products/muse-code);
...
| SAST                  | `semgrep scan --config auto --error`                                                       | **Deferred locally**: semgrep is not installed and has no supported Windows build; runs in CI on ubuntu (M0 CI job). |
...
M0–M6 and M8 certified; M7 certified or explicitly cut by the owner (Q3);
