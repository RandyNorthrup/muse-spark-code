...
| `taskkill /T` misses a child started during the kill (M27, found at the 0.6.0 gate) | `taskkill` kills the tree it enumerated: a child PowerShell started between that enumeration and its own end survived, orphaned. Under load it happened on every kill (the gate's `toolIo.test.ts` folder stayed held by the ping); one orphan was left suspended for good. | Once the shell has exited, `killTree` reads the process table (`Get-CimInstance Win32_Process` by `ParentProcessId`, Windows PowerShell with its own module path) and kills each orphan's tree, then looks for their orphans, three rounds at most. A child counts only if it was created after the shell started and before its parent died, so a later holder of a dead parent's id is never touched. A killed command's result waits for the sweep.                                                                                                                                                                                                                |
...
pull request from `hardening/m27-orphans`.
...
still there, suspended.

- **Goal**: a timed-out or stopped command ends with every child it
  started, on Windows as on POSIX (the D25 row above).
- **Scope**: `processTree.ts` (the sweep, `killTree` awaited),
  `runCommand` in `toolIo.ts` (the result waits for the kill), the
  constants, `processTree.test.ts`, `toolIo.test.ts`, the test folders'
  removal helper; README, CHANGELOG, this file.
- **Acceptance**: the escape reproduced for real (the shell killed alone,
  its ping found and killed); the sweep's rules over a scripted process
  table; the command's result waiting for it; five test-fire proofs; two
  full gate runs in a row green; the PR's CI green.
- **Security**: the sweep kills only processes created while their parent
  was the command's (never a later holder of a recycled id); it runs
  Windows PowerShell by absolute path under `%SystemRoot%`, with a fixed
  script and numeric ids only; no new dependency.
