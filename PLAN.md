...
| Q8  | Marketplace publisher: `vsce ls-publishers` shows no login on this machine and marketplace.visualstudio.com/manage needs an Azure DevOps sign-in. Owner to sign in (Chrome tab opened) so the publisher id can be read, then `npx vsce login <publisher>` with a Marketplace-scoped PAT. | Publisher stays the placeholder `randynorthrup` until then.       |
